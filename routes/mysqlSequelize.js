...
router.get('/item_types', async (req, res, next) => {
  const {model, attributes} = getModelAndAttributes.item_types();
  const result = await find(model, attributes)
  res.status(getResultStatus(result)).json(resultsDataToArray(result))
})

router.get('/totals_by_country', async (req, res, next) => {
...
router.get('/totals_by_region', async (req, res, next) => {
...
router.get('/item_type_by_country', async (req, res, next) => {
  const { country } = req.query
...
router.get('/item_type_by_region', async (req, res, next) => {
  const { region } = req.query
